...
═══════════════════════════════════════════════════════════════════ */   
    ...
    (function (global) {
      ...
      class ToolskinStylesheetDebugger {
        ...
        _build() {
          ...
          diffSelectsWrap.innerHTML =
            '<label><span class="ts-text-muted">Sheet A</span><div class="ts-ui-select" data-ts-ui-select-placeholder="Stylesheet A"><select class="ts-ui-select__native" data-role="diff-a" placeholder="Select Stylesheet A"  alt="Select Stylesheet A"></select></div></label>' +
            '<label><span class="ts-text-muted">Sheet B</span><div class="ts-ui-select" data-ts-ui-select-placeholder="Stylesheet B"><select class="ts-ui-select__native" data-role="diff-b" placeholder="Select Stylesheet B"  alt="Select Stylesheet B"></select></div></label>';
        ...
        openDiff(filePathA, filePathB) {
          ...
          controls.innerHTML =
            ...
            '<button type="button" class="ts-btn ts-btn--secondary ts-btn--sm" data-action="exit">Exit diff</button>';
      ...
      function maybeInstallDiffBridge() {
        ...
        // --- NEW: Apply stylesheet override if present ---
        const overridePath = params.get('__ts_override_css');
        if (overridePath) {
          // Resolve the full URL using the asset registry (if available)
          const reg = window.Toolskin && window.Toolskin.assets;
          if (reg && !reg.degraded) {
            // Find the record by path
            const record = reg.all().find(f => f.path === overridePath);
            if (record) {
              const url = reg.url(record);
              ...
              console.warn('[Diff] Override path not found in manifest:', overridePath);
            }
          } else {
            // Fallback: try to load directly (relative to basePath)
            const basePath = window.Toolskin && window.Toolskin.assets ? window.Toolskin.assets.basePath : 'assets/css/';
            const link = document.createElement('link');
            link.rel = 'stylesheet';
            link.href = basePath + overridePath;
            document.head.appendChild(link);
          }
        }

        // --- Original diff bridge code (scroll sync) ---
        ...
        // Initial post once DOM is ready
        if (document.readyState === 'loading') {
